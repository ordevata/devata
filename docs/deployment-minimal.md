...
### 2.3 Быстрый bootstrap (опционально)

Чтобы автоматизировать действия разделов 2–2.2, скопируйте на сервер скрипт
`infra/scripts/bootstrap-server.sh` и выполните его от имени `root` (или через
`sudo`):
...
Скрипт установит Docker и его репозиторий, настроит UFW (22/80/443) и создаст
каталог `/opt/devata` c подпапкой `db-backups`. После выполнения перезайдите в
SSH, чтобы применились права группы `docker`.
